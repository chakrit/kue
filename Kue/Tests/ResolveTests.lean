...
theorem resolve_reference_inside_conjunction :
    (resolveStructRefs
      (mkStruct [⟨"#A", .definition, .kind .int, false⟩, ⟨"x", .regular, .conj [.ref "#A", .boundConstraint (.int 0 .number) .ge], false⟩] .regularOpen none [])
      == mkStruct [⟨"#A", .definition, .kind .int, false⟩, ⟨"x", .regular, .conj [.refId ⟨0, 0⟩, .boundConstraint (.int 0 .number) .ge], false⟩] .regularOpen none []) = true := by
  native_decide
