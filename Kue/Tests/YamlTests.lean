...
theorem yaml_marshal_trailing_newline :
    (valueToYaml (mkStruct [⟨"a", .regular, .prim (.int 1), false⟩,
                           ⟨"b", .regular, .list [.prim (.int 1), .prim (.int 2)], false⟩] .regularOpen none [])).toOption
      == some "a: 1\nb:\n  - 1\n  - 2\n" := by native_decide

theorem yaml_marshal_list :
    (valueToYaml (.list [mkStruct [⟨"a", .regular, .prim (.int 1), false⟩] .regularOpen none [],
                         mkStruct [⟨"b", .regular, .prim (.int 2), false⟩] .regularOpen none []])).toOption
      == some "- a: 1\n- b: 2\n" := by native_decide
