...
theorem meet_number_kind_with_integer_lower_bound :
    meet (.kind .number) (.boundConstraint (.int 0 .number) .ge) = .boundConstraint (.int 0 .number) .ge := by
  rfl

theorem meet_integer_upper_bound_with_number_kind :
    meet (.boundConstraint (.int 10 .number) .le) (.kind .number) = .boundConstraint (.int 10 .number) .le := by
  rfl

theorem meet_string_kind_with_integer_lower_bound_bottoms :
    meet (.kind .string) (.boundConstraint (.int 0 .number) .ge) = .bottomWith [.kindConflict .string .number] := by
  rfl

theorem join_number_kind_with_integer_strict_lower_bound_normalizes :
    join (.kind .number) (.boundConstraint (.int 0 .number) .gt) = .kind .number := by
  rfl

theorem join_integer_strict_upper_bound_with_number_kind_normalizes :
    join (.boundConstraint (.int 10 .number) .lt) (.kind .number) = .kind .number := by
  rfl

theorem number_kind_subsumes_integer_lower_bound :
    subsumes (.kind .number) (.boundConstraint (.int 0 .number) .ge) = true := by
  native_decide
