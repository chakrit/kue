...
-- BI-2: list.Sort / list.SortStable. The comparator is a `{x, y, less}` struct evaluated per
-- pair; `list.Ascending`/`Descending` are the predefined comparator values. End-to-end
-- (parse → resolve → eval), since the sort is driven from the eval layer (the pure `Builtin`
-- layer cannot evaluate the comparator).
