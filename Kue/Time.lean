...
/-- Whether `value` satisfies the string-format validator `fmt`. The single predicate behind
    ...
    predicates live in `Kue/Time.lean` and `Kue/Net.lean`. Exhaustive over `StringFormat`, so
    a new validator surface forces a dispatch decision here rather than silently passing. -/
