...
| invalid pattern label with no field to match (RX-2b) | v0.16.1 | `{[=~"a("]: int}` — invalid regex `a(`, NO field present | `{}` — tolerated, no error | `_|_` — the invalid pattern bottoms the struct | RE2 (the spec authority: *"the regular expression syntax is that accepted by RE2"*) treats `a(` as ill-formed (`missing closing )`) regardless of whether any field is matched against it. cue evaluates the pattern LAZILY — it errors only when a concrete field's label is tested (`{[=~"a("]: int, k: 1}` DOES error in cue, agreeing with Kue), so a field-less struct silently keeps the malformed constraint. That laziness is an eval-strategy artifact (same family as SC-2b); an invalid regex literal is malformed at parse time, so the struct carrying it is bottom. Kue bottoms eagerly the moment the concrete pattern is present. NB: with a field present, cue and Kue AGREE (both error). | `definitions/regex_invalid_pattern_label` (field-present, both agree); the field-less divergence is pinned in `LatticeTests` `rx2b_label_pattern_invalid_bottoms` |
